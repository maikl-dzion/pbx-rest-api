...
class ringgroups extends rest {
    ...
    function __construct($f3) {
        ...
        if(!$this->conn) {
           throw new Exception('Не удалось создать подключение к Ami-сервисам (502 Service Unavailable)', 502);
        ...
        try {
            ...
            throw new Exception('Не удалось подключиться к базе (500 Internal Server Error)', 500);
    ...
    public function put($f3) {
        ...
        // $input['extension_list'] = explode(',', $input['extension_list']);
        // lg($input);
