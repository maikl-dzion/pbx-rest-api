...
class queues extends rest {
    ...
    function __construct($f3) {
        ...
        // lg($this->staticmembers);
        // throw new Exception(json_encode($rows), 500);
    }
        ...
        if(isset($post['static_members'])) {
            foreach($post['static_members'] as $thisexten) {
                ...
                if( (!in_array($output, $this->allextensions)) ) {
                    throw new Exception('422 Unprocessable Entity', 422);
    ...
    public function get($f3, $from_child=0) {
        ...
        // throw new Exception(json_encode($this->staticmembers), 500);
        ...
        foreach($rows as $idx => $row) {
            ...
            if(isset($this->staticmembers[$row['extension']])) {
                ...
                foreach($el as $key => $memb) {
                    $allstaticmembers[$key]= $this->reduce_static_member($memb);
                }
    ...
    public function post($f3, $from_child=0) {
        ...
        $input = json_decode($f3->get('BODY'),true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new Exception('422 Unprocessable Entity (json_last_error)', 422);
            //header($_SERVER['SERVER_PROTOCOL'] . ' 422 Unprocessable Entity (json_last_error)', true, 422);
            //die();
        }

        // If post has an ID, fail, it will create a new resource with next available id
        if($f3->get('PARAMS.id') != '') {
            throw new Exception('404 Not Found (not PARAMS.id)', 404);
            //header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true, 404);
            //die();
        }

        // Get next queue number from queues_config table, filling gaps if any
        // $EXTEN = $this->getNewNextExten($db);
        ...
        $newExtenConflict = true;
        while($newExtenConflict) {
            // Check if extension number is valid and it has no collitions
            if($this->checkValidExtension($f3, $EXTEN)) {
                $newExtenConflict = false;
            } else {
                $EXTEN++;
            }
        }
    ...
    public function getLastNextExten($db) {
        $idfield = $this->id_field;
        $query = "SELECT cast($idfield AS unsigned)+1 AS extension 
                   FROM queues_config mo WHERE NOT EXISTS ";
        $query.= "(SELECT NULL FROM queues_config mi WHERE cast(mi.$idfield AS unsigned) = CAST(mo.$idfield AS unsigned)+ 1) ";
        $query.= "ORDER BY CAST($idfield AS unsigned) LIMIT 1";
        $rows  = $db->exec($query);
        $EXTEN = $rows[0]['extension'];
    ...
    private function checkValidExtension($f3, $extension) {
        ...
        // lg([$extension, $this->alldestinations]);
