...
class extensions extends rest
{
    ...
    function __construct($f3)
    {
        ...
        $errHandle = $f3->get('ERROR_CLASS');

        $this->ami = new asteriskmanager();
        $this->conn = $this->ami->connect("localhost", "admin", $mgrpass);
        if (!$this->conn) {
            throw new Exception('Не удалось создать объект класса "asteriskmanager"', 500);
        ...
        try {
            ...
            throw new Exception("Не удалось создать объект класса 'DB\SQL\Mapper' ($message)", 500);
    ...
    public function get($f3, $from_child = 0)
    {
        ...
        $query  = "SELECT u.*,
                          dev.description AS name, 
                          dev.tech, 
                          dev.dial, 
                          sip.data AS secret 
                          FROM users u ";
        $query .= "LEFT JOIN devices dev ON u.extension = dev.id
                   LEFT JOIN sip ON u.extension=sip.id WHERE sip.keyword='secret' ";
