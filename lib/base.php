...
abstract class Prefab
{
    static function instance(){
        ...
        if (!Registry::exists($class)) {
            ...
            $args    = func_get_args();
            $object = $args ? $reflect->newinstanceargs($args) : new $class;
            Registry::set($class, $object);
        }
    ...
    function __construct()
    {
        ...
        set_exception_handler(
            function ($obj) {
                ...
                $message = $obj->getMessage() . ' ' . '[' . $obj->getFile() . ':' . $obj->getLine() . ']';
                $this->error(500, $message, $obj->getTrace());
            }
        );
        ...
        // lg($check);
        // $this->errorHandle = new CustomErrorHandler();
    ...
    function error($code, $text = '', array $trace = null, $level = 0)
    {
        ...
        if ((!$handler ||
             ...
             !$this->hive['QUIET']) {
                ...
                // lg($_SERVER);
                $ajaxError = [
                    'message' => $this->hive['ERROR'],
                    'code'    => $code,
                    'text'    => $text,
                    'request_uri' => $req,
                ];

//                throw new Exception('gfhfgf', $code);
//                die();

                $errorMessage = json_encode(['error' => $ajaxError]);

//                if($this->hive['AJAX']) {
//                    $errorMessage = json_encode(['error' => $ajaxError]);
//                } else {
//
//                    $style   = $highlight ? '<style>' . $this->read($css) . '</style>'  : '';
//                    $trace   = $this->hive['DEBUG'] ? '<pre>' . $trace . '</pre>' . $eol  : '';
//                    $content = $this->encode($req .'  =======  '. $text);
//                    $errorMessage = "<!DOCTYPE html>{$eol}
//                                      <html>{$eol}
//                                        <head>
...
//                                                <h1>{$header}</h1>{$eol}
//                                                <p>REQUEST : {$req}</p>{$eol}
//                                                <p>ERROR CODE : {$code}</p>{$eol}
//                                                <div style='color:red; font-weight: bold'>ERROR MESSAGE : {$text}</div>{$eol}
//                                                {$trace}
//                                        </body>{$eol}
//                                    </html>";
//
//                }
    ...
    protected function routesConverted() {

        $routeKeys = array_keys($this->hive['ROUTES']);
        $routeValues = array_values($this->hive['ROUTES']);

        $pathList = []; // Match specific routes first
        ...
        array_multisort($pathList, SORT_DESC, $routeKeys, $routeValues);

        return array_combine($routeKeys, $routeValues);
    }
    ...
    function run()
    {
        ...
        $this->hive['ROUTES'] = $this->routesConverted();

        // Convert to BASE-relative URL
        $requestUri = urldecode($this->hive['PATH']);

        $preflight = false;

        $cors = (isset($this->hive['HEADERS']['Origin']) &&
            $this->hive['CORS']['origin']) ? $this->hive['CORS'] : '';

        if ($cors) {
            $credentials = var_export($cors['credentials'], true);
        ...
        $allowed = [];
        $hiveRoutes = $this->hive['ROUTES'] ;

        foreach ($hiveRoutes as $pattern => $routes) {

            $args = $this->mask($pattern, $requestUri);
            ...
            $methodName = $this->hive['VERB'];

            if (isset($routes[$ptr][$methodName]) || isset($routes[$ptr = 0])) {
                $route = $routes[$ptr];
            }
            ...
            if (isset($route[$methodName]) && !$preflight) {

                if ($methodName == 'GET' && preg_match('/.+\/$/', $requestUri)) {
                    $_query = $this->hive['QUERY'] ? '?' . $this->hive['QUERY'] : '';
                    $uri    = substr($requestUri, 0, -1);
                    $this->reroute($uri . $_query);
                }

                list($handler, $ttl, $kbps, $alias) = $route[$methodName];

                $this->hive['PARAMS']  = $args; // Capture values of route pattern tokens
                $this->hive['ALIAS']   = $alias; // Save matching route
                $this->hive['PATTERN'] = $pattern;

                if ($cors && $cors['expose']) {
                    $expose = is_array($cors['expose']) ? implode(',', $cors['expose']) : $cors['expose'] ;
                ...
                if (is_string($handler)) {
                    ...
                    $preg = preg_match('/(.+)\h*(?:->|::)/', $handler, $match);

                    if ($preg && (!class_exists($match[1]))) {
                        // $this->error(404);
                        throw new Exception("Не найден класс-'{$match[1]}'", 404);
                    }

                }

                // dd(class_exists($match[1]));
                ...
                if (preg_match('/GET|HEAD/', $methodName) && $ttl) {
...
//                    }
    ...
    {
        $preg = preg_match('/(.+)\h*(->|::)\h*(.+)/s', $func, $parts);
        // lg($func, $args);
        ...
        if ($preg) {
            ...
            if (!class_exists($parts[1])) {
                throw new Exception("НЕ найден класс - '$parts[1]'", 404);
            ...
            if ($parts[2] == '->') {
                if (is_subclass_of($parts[1], 'Prefab')) {
                    ...
                    $parts[1] = $this->createController($parts[1], $args);
                }
            }
    ...
    protected function createController($className, $arguments = null) {
        ...
        if (method_exists($className, '__construct')) {
            ...
            try {
                // lg($arguments);
                $controller = (!empty($arguments)) ?
                              $reflect->newInstanceArgs($arguments) :
                              $reflect->newInstance();
                // lg($controller);
            } catch (\ReflectionException $err) {
                throw new Exception('Не удалось создать объект контроллера', 500);
            }
        } else {
