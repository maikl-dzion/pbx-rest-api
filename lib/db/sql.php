	...
	**/
	function schema($table,$fields=NULL,$ttl=0) {
		...
		foreach ($cmd as $key => $val) {
            ...
            if (!preg_match('/' . $key . '/', $this->engine)) continue;

            $rows = [];

            // lg($key, $val);

            $record = $this->exec($val[0], NULL);
                ...
                if (!$fields || in_array($row[$val[1]], $fields)) {
                    ...
                    $rows[$row[$val[1]]] = [
                        ...
                        'default'  => is_string($row[$val[3]]) ?
                                      preg_replace('/^\s*([\'"])(.*)\1\s*/', '\2', $row[$val[3]]) : $row[$val[3]],
            ...
            if ($fw->CACHE && $ttl)
                $cache->set($hash, $rows, $ttl); // Save to cache backend
