...
class Mapper extends \DB\Cursor
{
    ...
    function find($filter = NULL, array $options = NULL, $ttl = 0)
    {
        ...
        if( ($options['group'])  && (!preg_match('/mysql|sqlite/', $this->engine)) ) {
            $query = $options['group'];
        } else {
            $query = implode(',', array_map([$this->db, 'quotekey'], array_keys($this->fields)) );
        }

        $query .= $adhoc;
        // lg($query);

        return $this->select($query, $filter, $options, $ttl);
    }
    ...
    function insert()
    {
        ...
        if ($fields) {
            ...
            $query = (preg_match('/mssql|dblib|sqlsrv/', $this->engine) &&
                      array_intersect(array_keys($pkeys), $ckeys) ? 'SET IDENTITY_INSERT ' . $this->table . ' ON;' : '') .
