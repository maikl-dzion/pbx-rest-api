...
class jwtauth
{
    ...
    function post($f3)
    {
        ...
        $user = trim($f3->get('POST.user_'));
        $password = trim($f3->get('POST.password_'));

        if (empty($user)) {
            $user = trim($f3->get('POST.username'));
            if (empty($user)) {
                $ajaxData = (array)json_decode($f3->hive()['BODY']);
                $user = (!empty($ajaxData['user'])) ? $ajaxData['user'] : '';
            }
        }

        if (!$password)
            $password = (!empty($ajaxData['password'])) ? $ajaxData['password'] : '';
        ...
        // lg(['user' => $user, 'pass' =>$password, 'pwd' => $this->pwd, $ajaxData]);

        if ($user == 'admin' && $password == trim($this->pwd)) {
