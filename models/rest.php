...
class rest {
    ...
    function __construct($f3) {
        ...
        try {
            ...
            // lg($this);
            // if($this->table == 'ivrs') $this->table = 'ivr_details';
            ...
            lg('ERROR line=' . __LINE__ . ' File=' . __FILE__);
    ...
    function get($f3, $from_child) {
        ...
        // lg($this->condition, $f3->get('PARAMS.id'));
        ...
        if($f3->get('PARAMS.id') == '') {
            ...
            $this->data->load(array($this->id_field.'= ?', $itemId));

            if ($this->data->dry()) {
                // lg($this->data);
                $errorMesage = ' 404 Not Found (get->data->dry)';
                // $error = json_encode(['message' => ' 404 Not Found (get->data->dry)', 'data' => $this->data]);
                throw new Exception($errorMesage, 404);
    ...
    function post($f3, $from_child) {
        ...
        if($f3->get('PARAMS.id') <> '') {
            throw new Exception(' 404 Not Found (get(PARAMS.id) <> "")', 404);
            //header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true, 404);
            // die();
        }
        ...
        try {
            ...
            if ($e->getCode() != 23000) {
                ...
                throw new Exception(' 409 Conflict (23000))-' . $errMessage, 409);
                //header($_SERVER['SERVER_PROTOCOL'] . ' 409 Conflict', true, 409);
            } else {
                // on other errors
                throw new Exception(' 404 (Pdo error)' . $errMessage, 405);
                // header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found', true, 404);
            }
            die();
        }
    ...
    public function getTableFields($tableName) {
         ...
         $fields = $this->db->exec($query);
         $result = [];
         foreach ($fields as $key => $item) {
             $fname = $item['COLUMN_NAME'];
             $result[$fname] = $item;
         }
         return $result;
    }

    protected function sqlInsertPrepare($fields, $data) {
        $_fields = $_values = $markers = [];
        foreach ($data as $fname => $value) {
            if((!isset($fields[$fname])) || (!$value))
                continue;
        ...
        $fieldsString = implode(",", $_fields);
        $valuesMarker = implode(",", $markers);
        $sql = " ({$fieldsString}) VALUES({$valuesMarker}) ";

        if((!$fieldsString) || (!$valuesMarker) || (empty($_values)))
            return false;
    ...
    public function jsonResponse($data, $fname = '', $code = 200) {
